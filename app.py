...
# Filter data based on selections
filtered_data = data[(data["Year"] == selected_year) & (data["Scenario"] == selected_scenario)]
county_data = filtered_data[filtered_data["County"] == selected_county]
...
if not county_data.empty:
    tc_value = county_data["TC"].values[0]
    st.sidebar.write(f"### {selected_county} County: {tc_value} Tropical Cyclones")
else:
    st.sidebar.write(f"### {selected_county} County: No data available")
